...
string railfence_decrypt(string message) {
    ...
    for (int i = 0; i < message.length(); ++i) {
        ...
        if (isalpha(message[i]) == false) {
            ...
            cout << "invalid input, don't include spaces" << endl;
            getline(cin, message);
            cout << message << endl;
        }
    }
    ...
    if (length % 2 == 0) {
        // If it is, divide the message into two halves and store them in list1 and list2
        int limit = length / 2;
        ...
        for (int i = 0; i < length3; ++i) {
            finallist = finallist + list1[i] + list2[i];
        }
        ...
        finallist = finallist + list1[finalChar];
